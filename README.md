...
一套基于 LLM 的动态叙事引擎的 SillyTavern / TauriTavern 插件（当前版本 v1.5.3）。
...
- **首页**：Logo 英雄区 + 功能一览行列表。
- **地图系统**：首页「游戏地图」卡片独立入口（电脑端独立工作台大窗 / 手机端面板视图），为当前角色卡制作地图——上传图片 → 裁剪背景 → 双击建点 / 拖动 / 命名，保存后随角色卡导入导出。
- **游戏模式**：双入口——「游戏地图」（当前角色地图展示：背景图 + 地点标记）与「游戏数据」（只读展示变量系统注入的变量，章节默认折叠、点开看内容、展开状态持久化）；游戏模式空态「去编辑地图」直达地图编辑器。
- **对话请求基础**：`chatCompletion`（OpenAI 兼容）。
- **剧情预筛（Story Gate）**：发送前预筛并注入事件正文，请求内携带当前变量表（游戏值 YAML，未使用变量系统时省略）供 AI 核对涉及数值 / 状态的触发条件；「重新生成」（retry）自动重放上一轮入选事件。失败轮（含「思考阶段耗尽输出预算」的空回复）在注入实录留下原文与失败原因。
- **跨扩展发送屏障**：与 SoulLink 并发协调发送前任务。
- **注入实录**：展示最近一轮剧情预筛的完整结果（含随请求发送的变量表与失败原因）。
- **变量系统**：玩家自定义变量系统，支持变量注册、双层存储、AI 自动维护、注入提示词；子变量支持区间 / 取值映射 / 公式三种派生方式（如 情欲 81~100 → 等级「鱼水之欢」→ 取值映射输出对应态度描述）。
- **剧情触发**：按变量当前值满足预设条件时确定性注入事件正文（不依赖 AI 判断）；事件可嵌套分类管理（级联启停），分类与事件均支持拖动排序（把手即拖 / 长按整行）与**双击进编辑**，分类拖动时子树随行。
...
│   ├── views-values.js    # 变量工作台 UI（变量树 + 变量注册 + 剧情触发 + 注入预览）
│   ├── map-data.js        # 地图系统：数据模型 / 角色卡绑定 / 设置兜底 / 哨兵补丁
│   ├── map-image.js       # 地图系统：图片读取 / 裁剪 / 降采样导出
│   ├── views-map.js       # 地图系统：首页独立入口 + 游戏模式地图展示 + 编辑器 + 裁剪弹层
...
node tests/game-ui.test.js # 游戏模式 UI：双入口切换 / 章节折叠与展开持久化 / 地图展示 / 去编辑跳转
