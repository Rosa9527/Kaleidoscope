...
一套基于 LLM 的动态叙事引擎的 SillyTavern / TauriTavern 插件（当前版本 v1.5.2）。
...
- **剧情预筛（Story Gate）**：发送前预筛并注入事件正文，请求内携带当前变量表（游戏值 YAML，未使用变量系统时省略）供 AI 核对涉及数值 / 状态的触发条件；「重新生成」（retry）自动重放上一轮入选事件。
- **跨扩展发送屏障**：与 SoulLink 并发协调发送前任务。
- **注入实录**：展示最近一轮剧情预筛的完整结果（含随请求发送的变量表）。
