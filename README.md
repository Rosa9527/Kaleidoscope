...
一套基于 LLM 的动态叙事引擎的 SillyTavern / TauriTavern 插件（当前版本 v1.5.6）。
...
- **剧情脉络（可视化工作台）**：树状工作台，支持节点 / 事件的新建、编辑、删除、导入导出与角色卡绑定；节点与事件均支持拖动排序（把手即拖 / **长按整行**），节点拖动时整棵子树随行，顺序随角色卡与 YAML 天然往返（v1.5.6）。
...
- **变量系统**：玩家自定义变量系统，支持变量注册、双层存储、AI 自动维护、常驻注入提示词；子变量支持区间 / 取值映射 / 公式三种派生方式（如 情欲 81~100 → 等级「鱼水之欢」→ 取值映射输出对应态度描述）。
- **剧情触发**：按变量当前值满足预设条件时确定性注入事件正文（不依赖 AI 判断），事件块**常驻注入**、数据一变即刷新（v1.5.5）；求值无副作用，事件效果与一次性关闭只在发送那一刻执行一次并锁定本轮；事件可嵌套分类管理（级联启停），分类与事件均支持拖动排序（把手即拖 / 长按整行）与**双击进编辑**，分类拖动时子树随行。
...
│   ├── row-drag.js        # 通用行拖动排序引擎（变量系统与剧情脉络共用）
...
node tests/values-trigger.test.js # 剧情触发：CRUD / 条件求值 / 事件效果 / 常驻注入 / 本轮锁定 / YAML
node tests/values-inject.test.js # 变量注入：文本构建 / 常驻刷新 / 清理 / 发送前兜底
